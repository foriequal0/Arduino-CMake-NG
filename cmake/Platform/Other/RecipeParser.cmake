...
function(parse_compiler_recipe_flags _board_id _return_var)
    ...
    while (TRUE)
        ...
        set(continue_resolve FALSE)
        foreach (final_element ${final_recipe})
            _resolve_recipe_property("${final_element}" "${_board_id}" resolved_again)
            if (NOT ("${final_element}" STREQUAL "${resolved_again}"))
                set(continue_resolve TRUE)
                break()
            endif()
        endforeach()

        if (continue_resolve)
            set(original_list "${final_recipe}")
            set(final_recipe "")
            continue()
        else()
            break()
